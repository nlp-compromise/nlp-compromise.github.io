...
const words = ref(tmp)
const example = `// pre-compile lookup words
...
const lookup = computed(() => {
  const found = doc.value.lookup(trie.value)
  let list = doc.value.if(found)
  // list.debug()
  return list.html({ '.show': found })
})
function recompile() {
  let terms = words.value.split(/\r?\n/).map(txt => txt.trim())
  terms = terms.filter(str => str)
  trie.value = nlp.compile(terms)
}
recompile()
    ...
    <Page bottom="40px">
      ...
      <div class="both">
        <div style="max-width: 500px; margin-right: 2rem">
          <Picker :choice="choice" v-model:doc="doc" title="state of the union:" />
        </div>
        <div style="text-align: left">
          <div class="f08 grey">phrases to find:</div>
          <TextArea width="180px" height="300px" v-model:value="words" size="0.9rem" />
          <button style="margin-top: 1rem; margin-left: 1rem; font-size: 0.9rem" @click="recompile">
            ↻ recompile
          </button>
        </div>
      </div>
