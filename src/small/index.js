...
class Small extends React.Component {
  ...
  tick() {
    ...
    if (!state.dirty && diff < this.duration) {
      requestAnimationFrame(this.tick);
      let alpha = diff / this.duration;
      this.setState({ progress: alpha });
    } else {
      console.log('done-');
  ...
  onType(e) {
    console.log('type');
    this.setState({ dirty: true, text: e.target.value, progress: 0 });
  }
  render() {
    let { css, state } = this;
    ...
    return (
      <div style={css.container}>
        <div style={css.inputContainer}>
          <textarea ref="input" style={{ ...css.input, height: height }} value={state.text} onChange={this.onType} />
          <div style={{ ...css.loader, width: state.progress * 100 + '%' }} />
        </div>
        <div style={{ fontSize: 35, color: '#cc9696' }}>👇</div>
